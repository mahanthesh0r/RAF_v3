...
class CamView(object):
    ...
    def actionKeySelection(self, item_cls):
        # Selection Parameters
        selected = False
        timer_running = False
        timer = 0
        ...
        print("Press 'a' for 'sip' or 'e' for grasp")
        ...
        while True:
            ...
            if user_input=='a':
                ...
                print("Invalid key. Press 'a' for 'sip' or 'e' for 'grasp'.")
        ...
        if run.raf_state.system_state == 'selection':
            ...
            if run.raf_state.enable_arm_detections:
                ...
                if run.raf_state.visualize_detections == "normal":
                    ...
                    selected_action = run.Selection(item, mouse_pos)
                ...
                if run.raf_state.visualize_detections == "normal":
                    ...
                    # print("Selected Item: ", item)
                    selected_action = run.actionKeySelection(item, mouse_pos)
                    if selected_action == 'Cancel':
                        run.request_pub.publish('system-state-selection')
                        rospy.sleep(0.1)
                    elif selected_action is not None:
